    ...
    for idx, row in comparison_gdf.iterrows():
        ...
        if common_columns:
            ...
            for existing_attrs in existing_records[geom_hash]:
                ...
                logger.debug(f"\nComparing record {idx}:")
                logger.debug(f"Geometry hash: {geom_hash}")
                
                for col in common_columns:
                    existing_val = existing_attrs.get(col)
                    current_val = current_attrs.get(col)
                    match = (pd.isna(existing_val) and pd.isna(current_val)) or (str(existing_val) == str(current_val))
                    logger.debug(f"Column '{col}': existing='{existing_val}' ({type(existing_val)}), current='{current_val}' ({type(current_val)}), match={match}")
                
                # Check if all attributes match
                attrs_match = all(
                    (pd.isna(existing_attrs.get(col)) and pd.isna(current_attrs.get(col))) or
                    (str(existing_attrs.get(col)).strip() == str(current_attrs.get(col)).strip())
                    for col in common_columns
                )
            ...
            if not match_found:
                ...
                logger.debug(f"Record marked as updated due to no matching attributes found")
        else:
            identical_geometries.append(row)
