...
def compare_geometries(gdf, conn, table_name, geom_column='geom'):
    ...
    cursor.close()
    return (
        GeoDataFrame(new_geometries, crs="EPSG:4326") if new_geometries else None,
        GeoDataFrame(updated_geometries, crs="EPSG:4326") if updated_geometries else None,
        GeoDataFrame(identical_geometries, crs="EPSG:4326") if identical_geometries else None
    )
