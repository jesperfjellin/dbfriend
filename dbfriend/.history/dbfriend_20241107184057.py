...
def create_generic_geometry_table(conn, engine, table_name, schema='public'):
    """Create a new table with a generic geometry column."""
    cursor = conn.cursor()
    try:
        # Create table with generic geometry type
        sql = f"""
        ...
        conn.commit()
        logger.info(f"Created new table '{schema}.{table_name}' with generic geometry type")
        return True
    except Exception as e:
        conn.rollback()
            ...
            try:
                ...
                if args.table:
                    ...
                    if table_name not in existing_tables:
                        # Create table with generic geometry type
                        if create_generic_geometry_table(conn, engine, table_name, schema):
                            existing_tables.append(table_name)
                        else:
                            continue
                        
                        # Append first batch of geometries
                        logger.info(f"Appending {len(gdf)} geometries to '{qualified_table}'")
                        gdf.to_postgis(
                            name=table_name,
                            con=engine,
                            schema=schema,
                        ...
                        new_geoms, updated_geoms, identical_geoms = compare_geometries(
                            gdf, conn, table_name, 'geom', exclude_columns=[], args=args
                        )
                        ...
                        if new_geoms is not None and not new_geoms.empty:
                            logger.info(f"Appending {len(new_geoms)} new geometries to '{qualified_table}'")
