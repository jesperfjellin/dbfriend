...
def get_non_essential_columns(conn, table_name):
    """
    Retrieve a list of non-essential columns such as primary keys, timestamps, etc.
    Adjust the logic based on your table's schema.
    """
    cursor = conn.cursor()
    cursor.execute(f"""
        SELECT column_name 
    ...
    # Define patterns or specific column names to exclude
    exclude_patterns = ['id', 'gid', 'osm_id', 'created_at', 'updated_at', 'uuid']
    exclude_columns = set()
...
def process_files(args, conn, existing_tables):
    ...
    if not file_info_list:
        logger.warning("No spatial files found to process.")
        return
