    ...
    if len(geom_col_files) == 1:
        ...
        if geom_col != 'geom':
            ...
            if args.rename_geom:
                ...
                action = console.input(f"[red]Geometry column detected as '{geom_col}' for files {file_names}. Rename to 'geom'? (y/n): [/yellow]")
            if action.lower() == 'y':
                for info in file_info_list:
        ...
        for geom_col, files in geom_col_files.items():
            if geom_col != 'geom':
                if args.rename_geom:
                    ...
                    action = console.input(f"[red]Geometry column detected as '{geom_col}' for files {files}. Rename to 'geom'? (y/n): [/yellow]")
                if action.lower() == 'y':
                    for info in file_info_list:
