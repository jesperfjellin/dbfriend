...
def compare_geometries(gdf, conn, table_name, geom_column='geom'):
    ...
    cursor.execute(f"""
        SELECT 
            ...
            MD5(CAST(row_to_json((SELECT r FROM (SELECT {columns_sql}) r)) AS TEXT)) as attr_hash
        FROM "{table_name}"
        WHERE "{geom_column}" IS NOT NULL
    """)
    
    # Store both geometry and attribute hashes
    existing_records = {}  # geom_hash -> attr_hash
    ...
    # Compute attribute hashes
    comparison_gdf['attr_hash'] = comparison_gdf.apply(
        lambda row: hashlib.md5(
            json.dumps({col: row[col] for col in common_columns}, sort_keys=True).encode()
        ).hexdigest(),
        axis=1
    )
    
    # Identify new, updated, and identical geometries
    new_mask = ~comparison_gdf['geom_hash'].isin(existing_records.keys())
    new_geometries = gdf[new_mask].copy()
    
    existing_mask = comparison_gdf['geom_hash'].isin(existing_records.keys())
    existing_geoms = comparison_gdf[existing_mask]
    
    # For existing geometries, check if attributes match
    updated_mask = existing_geoms.apply(
        lambda row: existing_records[row['geom_hash']] != row['attr_hash'],
        axis=1
    )
    
    updated_geometries = gdf[existing_mask][updated_mask].copy()
    identical_geometries = gdf[existing_mask][~updated_mask].copy()
    
    cursor.close()
    
    return (
        new_geometries if not new_geometries.empty else None,
        updated_geometries if not updated_geometries.empty else None,
        identical_geometries if not identical_geometries.empty else None
    )
...
def process_files(args, conn, existing_tables):
    ...
    file_info_list = []
    processed_files = set()  # Track unique files
