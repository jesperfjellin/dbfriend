    ...
    with Progress(
        ...
        for info in file_info_list:
            ...
            if table_name in existing_tables:
                ...
                # Log the number of identical geometries found
                if identical_geoms is not None and not identical_geoms.empty:
                    num_identical = len(identical_geoms)
                    if num_identical > 0:
                        logger.info(f"Found {num_identical} identical geometries in {table_name}. Skipping...")
                    else:
                        logger.info(f"No identical geometries found in {table_name}")
                else:
                    logger.info(f"No identical geometries found in {table_name}")
                
                # Handle new geometries
                if new_geoms is not None and not new_geoms.empty:
                    num_new = len(new_geoms)
                    if num_new > 0:
                        logger.info(f"Found {num_new} new geometries to append to {table_name}")
                        try:
