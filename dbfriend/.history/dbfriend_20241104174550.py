    ...
    for idx, row in comparison_gdf.iterrows():
        ...
        if common_columns:
            ...
            for existing_attrs in existing_records[geom_hash]:
                ...
                if attrs_match:
                    logger.debug(f"Identical geometry found with hash {geom_hash}")
                    identical_geometries.append(row)
                    found_match = True
                    break
            
            # If no exact match found, this is an update
            if not found_match:
                logger.debug(f"Updated geometry found with hash {geom_hash}")
